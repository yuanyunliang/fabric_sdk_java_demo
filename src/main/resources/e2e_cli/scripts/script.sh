...
CHAINCODE_NAME="$2"
: ${CHANNEL_NAME:="chaincode_example02"}
VERSION="$3"
: ${VERSION:="1.0"}
...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n $CHAINCODE_NAME -v $VERSION -p github.com/hyperledger/fabric/examples/chaincode/go/$CHAINCODE_NAME >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CHAINCODE_NAME -v $VERSION -c '{"Args":["initTest"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
...
upgradeChaincode () {
	...
	peer chaincode upgrade -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CHAINCODE_NAME -v $VERSION -c '{"Args":["initTest"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
...
if [[ $4 -eq 1 ]] ; then
	...
	echo "Upgrade chaincode on org2/peer2..."
	upgradeChaincode 2
else
