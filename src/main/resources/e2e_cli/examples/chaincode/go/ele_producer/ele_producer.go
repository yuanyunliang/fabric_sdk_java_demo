...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	if function == "initTest" {
		return s.initTestEnergyProducer(stub)
...
func (s *SmartContract) initTestEnergyProducer(stub shim.ChaincodeStubInterface) sc.Response {
	EnergyProducers := []EnergyProducer{
	...
	i := 0
	for i < len(EnergyProducers) {
		fmt.Println("i is ", i)
		energyProducersAsBytes, _ := json.Marshal(EnergyProducers[i])
		stub.PutState("EP"+strconv.Itoa(i), energyProducersAsBytes)
		fmt.Println("Added", EnergyProducers[i])
		i = i + 1
	}
	return shim.Success(nil)
}
...
func (s *SmartContract) invoke(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var energyProducer = EnergyProducer{MeterId: args[1], Electricity: args[2], StartTime: args[3], EndTime: args[4]}
	fmt.Println("New Added:", energyProducer)

	energyProducerAsBytes, _ := json.Marshal(energyProducer)
	fmt.Println("New args[0]:", "EP"+args[0])
	fmt.Println("New EnergyProducerAsBytes:", energyProducerAsBytes)

	stub.PutState("EP"+args[0], energyProducerAsBytes)
...
func (s *SmartContract) query(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	energyProducerAsBytes, _ := stub.GetState("PC"+args[0])
	return shim.Success(energyProducerAsBytes)
}
...
func (s *SmartContract) queryAll(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	fmt.Printf("- listAllEnergyProducer:\n%s\n", buffer.String())
